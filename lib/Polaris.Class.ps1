...
class Polaris {
    ...
    static [void] Send (
        ...
        $RawResponse.StatusCode = $StatusCode        $RawResponse.Headers = $Headers        if ($ByteResponse.Length -gt 0) {
            $RawResponse.ContentType = $ContentType
        }
        $RawResponse.ContentLength64 = $ByteResponse.Length        $RawResponse.OutputStream.Write($ByteResponse, 0, $ByteResponse.Length)        $RawResponse.OutputStream.Close()    
    }
    ...
    static [void] Send (
        ...
        $RawResponse.StatusCode = $StatusCode        $RawResponse.Headers = $Headers        $RawResponse.ContentType = $ContentType        $StreamResponse.CopyTo($RawResponse.OutputStream)        $RawResponse.OutputStream.Close()    
    }
